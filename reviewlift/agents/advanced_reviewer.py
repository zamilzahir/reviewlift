...
class AdvancedReviewerAgent:
    """Calls the Claude API (Haiku 4.5) to review a code diff for complex logic and
    security issues. The only tier that costs real money. Retries once on transient
    failure before falling back to an error result."""
...
"""
        ...
        try:
            ...
            print(f"[advanced_reviewer] fallback after retries due to: {e}")
            return ReviewResult(
                findings=[], confidence=0.0, model_used="advanced_error",
